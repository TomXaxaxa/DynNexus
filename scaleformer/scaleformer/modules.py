"""Core neural network modules used by ScaleFormer."""
...
class DyT(nn.Module):
    """Learnable tanh-based channel-wise affine transform."""
...
class PatchTSTKernelEmbedding(nn.Module):
    """Kernel-feature embedding for patch inputs."""
...
class PatchTSTPolynomialEmbedding(nn.Module):
    """Polynomial-plus-RFF embedding variant for patch inputs."""
...
class PatchTSTRMSNorm(nn.Module):
    """RMSNorm layer adapted for PatchTST hidden states."""
...
def apply_p_rope_to_qk(
    ...
    """Apply p-RoPE positional embedding to query/key states.
...
class PatchTSTFourierApproximator(nn.Module):
    """Approximate trajectories using top-k Fourier modes."""
