"""Evaluation entrypoint for ScaleFormer checkpoints."""
...
def main(cfg):
    """Run model evaluation and optionally persist trajectories and labels."""
    save_eval_artifacts = cfg.eval.save_eval_artifacts
    results_dir = cfg.eval.results_dir
    metrics_save_dir = Path(results_dir) / "metrics"
    forecast_save_dir = Path(results_dir) / "forecast"
    labels_save_dir = Path(results_dir) / "labels"
    ...
    save_eval_results_fn = partial(
        ...
        metrics_save_dir=str(metrics_save_dir),
    ...
    log(f"Saving evaluation results to {results_dir}")
    ...
    predictions, contexts, labels, metrics = evaluate_forecasting_model(
        ...
        return_predictions=save_eval_artifacts,
        return_contexts=save_eval_artifacts,
        return_labels=save_eval_artifacts,
    ...
    if save_eval_artifacts:
        assert predictions is not None and contexts is not None and labels is not None
        process_trajs_fn(
            str(forecast_save_dir),
